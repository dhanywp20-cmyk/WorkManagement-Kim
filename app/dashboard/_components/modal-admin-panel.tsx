...
export function AdminPanelModal({ initialTab, onClose }: AdminPanelModalProps) {
  const [activeSection, setActiveSection] = useState<'settings' | 'userManagement' | 'merek' | 'kelompok' | 'integrasi'>(initialTab);
...
const navItems: { key: 'settings' | 'userManagement' | 'merek' | 'kelompok' | 'integrasi'; group: typeof GRUP_NAV[number]['key']; label: string; icon: React.ReactElement; color: string; activeBg: string; activeBorder: string; activeText: string }[] = [
    {
      key: 'settings',
      group: 'organization',
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 flex items-center gap-2 sm:gap-3 flex-shrink-0"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-slate-500 text-[10px] sm:text-xs hidden sm:block">
                ...
                {activeSection === 'kelompok' && 'Daftar kelompok kerja - sumber dropdown Tipe PTS di form akun'}
